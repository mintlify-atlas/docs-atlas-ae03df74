...
rk install-skill --ide cursor --project   # project-scoped install
```

Omitting `--ide` installs the skill globally for the default detected environment. The `--project` flag scopes the install to the current repository rather than the user's global config.
