...
For production workloads, you can add per-state caps:
...
`maxConcurrentSprintsByState.review: 1` is a deliberate bottleneck — reviews require human attention and piling them up creates a backlog. Tune `active` to match your agent budget and machine capacity.
